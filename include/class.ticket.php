...
class Ticket extends VerySimpleModel implements RestrictedAccess, Threadable, Searchable
{
    ...
    static protected $sources = array(
        'Messengers' =>
        /* @trans */    'Messengers (Whatsapp/Telegram)',
        'Phone' =>
        /* @trans */    'Phone',
        'Email' =>
        /* @trans */    'Email',
    ...
    function loadDynamicData($force = false)
    {
        if (!isset($this->_answers) || $force) {
            $this->_answers = array();
            foreach (DynamicFormEntryAnswer::objects()->filter(array('entry__object_id' => $this->getId(), 'entry__object_type' => 'T')) as $answer) {
    ...
    function isCloseable()
    {
        ...
        if (self::getMissingRequiredFields($this)) {
            ...
            $warning = sprintf(
                __('%1$s has %2$d open tasks and cannot be closed'),
                __('This ticket'),
                $num
            );
    ...
    {
        return count($this->tasks->filter(
            array(
    ...
    function getRecipients($who = 'all', $whitelist = array(), $active = true)
    {
        ...
        switch (strtolower($who)) {
            ...
            case 'collabs':
                if (
                    ($collabs = $active ? $this->getActiveCollaborators() :
    ...
    function getAssignmentForm($source = null, $options = array())
    {
        ...
        if (($refer = $form->getField('refer'))) {
            if (!$assignee) {
                ...
                $refer->configure('desc', sprintf(
                    __('Maintain referral access to %s'),
                    $this->getAssigned()
                )
                );
            }
    ...
    function getField($fid)
    {
        ...
        switch ($fid) {
            ...
            case 'duedate':
                ...
                $hint = sprintf(
                    __('Setting a %s will override %s'),
                    __('Due Date'),
                    __('SLA Plan')
                );
                return DateTimeField::init(
                    array(
                        'id' => $fid,
                        'name' => $fid,
    ...
    function getDynamicFields($criteria = array())
    {
        ...
        $fields = DynamicFormField::objects()->filter(
            array(
                'id__in' => $this->entries
                    ->filter($criteria)
    ...
    function addCollaborator($user, $vars, &$errors, $event = true)
    {
        ...
        if (
            $user && !$errors
            && ($c = $this->getThread()->addCollaborator(
                $user,
                $vars,
                $errors,
                $event
    ...
    function updateCollaborators($vars, &$errors)
    {
        ...
        if ($vars['cid'] && ($cids = array_filter($vars['cid']))) {
            $this->getThread()->collaborators->filter(
                array(
                    'thread_id' => $this->getThreadId(),
                    'id__in' => $cids
                )
    ...
    function getAuthToken($user, $algo = 1)
    {
        ...
        switch ($algo) {
            case 1:
                $authtoken .= substr(
                    base64_encode(
                        md5($user->getId() . $this->getCreateDate() . $this->getId() . SECRET_SALT, true)
                    ), 8);
                break;
            default:
                return null;
    ...
    function sendAccessLink($user)
    {
        ...
        $msg = $ost->replaceTemplateVariables(
            array(
        ...
        $email->send(
            $user, Format::striptags($msg['subj']),
            $msg['body']
        );
    }
    ...
    function setStatus($status, $comments = '', &$errors = array(), $set_closing_agent = true, $force_close = false)
    {
        ...
        if (
            (!$status instanceof TicketStatus)
    ...
    function onNewTicket($message, $autorespond = true, $alertstaff = true)
    {
        ...
        $options = array();
        if (
            ($message instanceof ThreadEntry)
        ...
        if (
            ...
            $email->sendAutoReply(
                $this->getOwner(), $msg['subj'], $msg['body'],
                null,
                $options
            );
        }
        ...
        if (
            ...
            if ($dept->getNumMembersForAlerts()) {
                ...
                if (
                    $cfg->alertDeptMembersONNewTicket() && !$this->isAssigned()
                ...
                // Account manager
                if (
                    $cfg->alertAcctManagerONNewTicket()
    ...
    function onOpenLimit($sendNotice = true)
    {
        ...
        $msg = sprintf(
            _S('Maximum open tickets (%1$d) reached for %2$s'),
            $cfg->getMaxOpenTickets(), $this->getEmail()
        );
        $ost->logWarning(
            sprintf(_S('Maximum Open Tickets Limit (%s)'), $this->getEmail()),
            $msg
        );
    ...
    function notifyCollaborators($entry, $vars = array())
    {
        ...
        $email->send(
            $recipients, $cnotice['subj'], $cnotice['body'],
            $attachments,
            $options
        );
    }
    ...
    function onMessage($message, $autorespond = true, $reopen = true)
    {
        ...
        if ($this->getOwnerId() == $message->getUserId())
            $user = new TicketOwner(
                User::lookup($message->getUserId()),
                $this
            );
        else
            $user = Collaborator::lookup(
                array(
    ...
    function onAssign($assignee, $comments, $alert = true)
    {
        ...
        if ($user_comments) {
            if (
                ...
                $title = sprintf(
                    _S('Ticket claimed by %s'),
                    $thisstaff->getName()
                );
            else
                $title = sprintf(
                    _S('Ticket Assigned to %s'),
                    $assignee->getName()
                );
    ...
    function onOverdue($whine = true, $comments = "")
    {
        ...
        if (
            ...
            if (
                $cfg->alertDeptManagerONOverdueTicket()
    ...
    static function getVarScope()
    {
        $base = array(
            ...
            'recipients' => array(
                'class' => 'UserList',
                'desc' => __('List of all recipient names'),
            ),
            'source' => __('Source'),
            'status' => array(
                'class' => 'TicketStatus',
            ...
            'staff' => array(
                'class' => 'Staff',
                'desc' => __('Assigned/closing agent'),
            ),
            'subject' => 'Subject',
            'team' => array(
                'class' => 'Team',
                'desc' => __('Assigned/closing team'),
            ),
            'thread' => array(
                'class' => 'TicketThread',
                'desc' => __('Ticket Thread'),
            ),
            'topic' => array(
                'class' => 'Topic',
                'desc' => __('Help Topic'),
            ...
            'last_update' => array(
                'class' => 'FormattedDate',
                'desc' => __('Time of last update'),
            ),
            'user' => array(
                'class' => 'User',
                'desc' => __('Ticket Owner'),
            ),
        );
    ...
    static function getSearchableFields()
    {
        ...
        $base = array(
            ...
            'assignee' => new AssigneeChoiceField(
                array(
                    'label' => __('Assignee'),
                )
            ),
            'staff_id' => new AgentSelectionField(
                array(
                    'label' => __('Assigned Staff'),
                    'configuration' => array('staff' => $thisstaff),
                )
            ),
            'team_id' => new TeamSelectionField(
                array(
                    'label' => __('Assigned Team'),
                )
            ),
            'dept_id' => new DepartmentChoiceField(
                array(
                    'label' => __('Department'),
                )
            ),
            'sla_id' => new SLAChoiceField(
                array(
                    'label' => __('SLA Plan'),
                )
            ),
            'topic_id' => new HelpTopicChoiceField(
                array(
                    'label' => __('Help Topic'),
                )
            ),
            'source' => new TicketSourceChoiceField(
                array(
                    'label' => __('Ticket Source'),
                )
            ),
            'isoverdue' => new BooleanField(
                array(
                    'label' => __('Overdue'),
            ...
            'isassigned' => new AssignedField(
                array(
                    'label' => __('Assigned'),
                )
            ),
            'merged' => new MergedField(
                array(
                    'label' => __('Merged'),
                )
            ),
            'linked' => new LinkedField(
                array(
                    'label' => __('Linked'),
                )
            ),
            'thread_count' => new TicketThreadCountField(
                array(
                    'label' => __('Thread Count'),
                )
            ),
            'attachment_count' => new ThreadAttachmentCountField(
                array(
                    'label' => __('Attachment Count'),
                )
            ),
            'collaborator_count' => new ThreadCollaboratorCountField(
                array(
                    'label' => __('Collaborator Count'),
                )
            ),
            'task_count' => new TicketTasksCountField(
                array(
                    'label' => __('Task Count'),
                )
            ),
        ...
        if ($tickets['dtids']) {
            ...
            foreach ($tickets['tids'] as $key => $value) {
                if ($ticket = Ticket::lookupByNumber($value)) {
                    ...
                    if (
                        ($ticket->isParent() || $ticket->isChild()) &&
                        $ticket->getMergeType() == 'visual' && $tickets['combine'] != 2 ||
                        ($tickets['combine'] == 2 && !$parent->isParent() && $parent->isChild())
                    ) { //changing link parent
                        $ticket->unlink();
                        $changeParent = true;
                    }
                    ...
                    if ($parent && $parent->getId() != $ticket->getId()) {
                        if (
                            ($changeParent) || ($parent->isParent() && $ticket->getMergeType() == 'visual' && !$ticket->isChild()) || //adding to link/merge
                            (!$parent->isParent() && !$ticket->isChild())
                        ) { //creating fresh link/merge
    ...
    function transfer(TransferForm $form, &$errors, $alert = true)
    {
        ...
        elseif ($dept->getid() == $this->getDeptId())
            $errors['dept'] = sprintf(
                __('%s already in the department'),
                __('Ticket')
            );
        else {
        ...
        switch (true) {
            case $referee instanceof Staff:
                ...
                if ($this->getStaffId() == $referee->getId()) {
                    $errors['agent'] = sprintf(
                        __('%s is assigned to %s'),
                        __('Ticket'),
                        __('the agent')
                    );
                } elseif (!$referee->isAvailable()) {
                    $errors['agent'] = sprintf(
                        __('Agent is unavailable for %s'),
                        __('referral')
                    );
                } else {
    ...
    function postMessage($vars, $origin = '', $alerts = true)
    {
        ...
        if ($vars['userId'] != $ticket->user_id) {
            ...
            if ($user) {
                $v = array();
                $c = $ticket->getThread()->addCollaborator(
                    $user,
                    $v,
                    $errors
                );
        ...
        if (
            ...
            // Dept manager
            if (
                $cfg->alertDeptManagerONNewMessage()
                && $dept
                && ($manager = $dept->getManager())
            ) {
                $recipients[] = $manager;
            ...
            // Account manager
            if (
                $cfg->alertAcctManagerONNewMessage()
                && ($org = $this->getOwner()->getOrganization())
                && ($acct_manager = $org->getAccountManager())
            ) {
                if ($acct_manager instanceof Team)
    ...
    function postCannedReply($canned, $message, $alert = true)
    {
        ...
        if (
            (!is_object($canned) && !($canned = Canned::lookup($canned)))
            || !$canned->isEnabled()
        ) {
            return false;
        }
        ...
        $info = array(
            'msgId' => $message instanceof ThreadEntry ? $message->getId() : 0,
    ...
    function update($vars, &$errors)
    {
        ...
        if (
            isset($vars['source']) // Check ticket source if provided
            && !array_key_exists($vars['source'], Ticket::getSources())
        )
            $errors['source'] = sprintf(
                __('Invalid source given - %s'),
    ...
    function updateField($form, &$errors)
    {
        ...
        if (!($changes = $field->getChanges()))
            $errors['field'] = sprintf(
                __('%s is already assigned this value'),
                __($field->getLabel())
            );
        else {
            if ($field->answer) {
    ...
    */
    ...
    protected static function filterTicketData($origin, $vars, $forms, $user = false, $postCreate = false)
    {
        ...
        foreach ($vars as $k => $v)
            if (strpos($k, 'field.') === 0)
                unset($vars[$k]); foreach ($forms as $F) {
            if ($F) {
    ...
    )
    {
        ...
        if (!$errors) {
            ...
            if ($vars['topicId']) {
                if ($__topic = Topic::lookup($vars['topicId'])) {
                    foreach ($__topic->getForms() as $idx => $__F) {
                        ...
                        $__F->extra = JsonDataEncoder::encode(
                            array(
                                'disable' => $disabled
                            )
                        );
            ...
            try {
                $vars = self::filterTicketData(
                    $origin,
                    $vars,
                    array_merge(array($form), $topic_forms),
                    $user,
                    false
                );
                ...
                return $reject_ticket(
                    sprintf(
                        _S('Ticket rejected (%s) by filter "%s"'),
            ...
            if (
                ...
                $ost->logWarning(
                    ...
                    sprintf(
                        _S('Max open tickets (%1$d) reached for %2$s'),
            ...
            if (!$user) {
                ...
                if (
                    ...
                    if (!Organization::forDomain($domain)) {
                        return $reject_ticket(
                            sprintf(
                                _S('Ticket rejected (%s) (unregistered client)'),
                                $vars['email']
                            )
                        );
                    }
        ...
        if ($_SESSION[':form-data']['ticketId'] || $_SESSION[':form-data']['taskId']) {
            ...
            $ticketNote = array(
                'title' => __('Ticket Created From Thread Entry'),
                'body' => sprintf(
                    __('Ticket %1$s<br/> Thread Entry: %2$s'),
                    $ticketLink,
                    $entryLink
                )
            );
        ...
        if ($ticket->isOpen()) {
            ...
            if ($vars['assignId']) {
                $asnform = $ticket->getAssignmentForm(
                    array(
